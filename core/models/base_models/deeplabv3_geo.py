...
#最后修改
__all__ = ['DeepLabV3_Geo', 'get_deeplabv3', 'get_deeplabv3_resnet50_voc', 'get_deeplabv3_resnet101_voc',
           'get_deeplabv3_resnet152_voc', 'get_deeplabv3_resnet50_ade', 'get_deeplabv3_resnet101_ade',
           'get_deeplabv3_resnet152_ade']


class DeepLabV3_Geo(SegBaseModel):
    r"""DeepLabV3_Geo
    ...
    def __init__(self, nclass, backbone='resnet50', aux=False, pretrained_base=True, **kwargs):
        super(DeepLabV3_Geo, self).__init__(nclass, aux, backbone, pretrained_base=pretrained_base, **kwargs)
        self.head = _DeepLabHead_Geo(nclass, **kwargs)
...
class _DeepLabHead_Geo(nn.Module):
    def __init__(self, nclass, norm_layer=nn.BatchNorm2d, norm_kwargs=None, **kwargs):
        ...
        self.block2 = nn.Sequential(
            nn.Conv2d(258, 258, 3, padding=1, bias=False),
            norm_layer(258, **({} if norm_kwargs is None else norm_kwargs)),
            nn.ReLU(True),
            nn.Dropout(0.1),
            nn.Conv2d(258, 2, 1)
        )
        self.block3 = nn.Sequential(
            nn.Conv2d(260, 260, 3, padding=1, bias=False),
            norm_layer(260, **({} if norm_kwargs is None else norm_kwargs)),
            nn.ReLU(True),
            nn.Dropout(0.1),
            nn.Conv2d(260, nclass-2, 1)
        )
    def forward(self, x):
        x = self.aspp(x)
        x_1 = self.block1(x)
        x_2 = self.block2(torch.cat([x, x1], dim=1))
        x_3 = self.block3(torch.cat([x,x1,x2],dim=1))
        return x_1,x_2,x_3
