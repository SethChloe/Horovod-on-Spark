...
class DeepLabV3(SegBaseModel):
    ...
    def __init__(self, nclass, backbone='resnet50', aux=False, pretrained_base=True, **kwargs):
        ...
        if self.aux:
            #这里改成了2048，因为加入了纹理特征等
            self.auxlayer = _FCNHead(2048, nclass, **kwargs)
    ...
    def forward(self, x, img_feat = None):
        ...
        if img_feat is not None:
            ...
            c4 = torch.cat([c4, c4_img_feat],dim=1)
            c3 = torch.cat([c3, c3_img_feat],dim=1)
        outputs = []
        x = self.head(c4)
...
class _DeepLabHead(nn.Module):
    def __init__(self, nclass, norm_layer=nn.BatchNorm2d, norm_kwargs=None, **kwargs):
        ...
        #这里改成了4096
        self.aspp = _ASPP(4096, [12, 24, 36], norm_layer=norm_layer, norm_kwargs=norm_kwargs, **kwargs)
