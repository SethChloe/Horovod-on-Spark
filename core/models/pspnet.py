...
class PSPNet(SegBaseModel):
    ...
    def __init__(self, nclass, backbone='resnet50', aux=False, pretrained_base=True, **kwargs):
        ...
        if self.aux:
            self.auxlayer = _FCNHead(1026, nclass, **kwargs)
        self.__setattr__('exclusive', ['head', 'auxlayer'] if aux else ['head'])

    def forward(self, x, lbr = None):
        size = x.size()[2:]
        _, _, c3, c4 = self.base_forward(x)
        if lbr is not None:
            lbr = self.pool(lbr)
            #_, _, c3_img_feat, c4_img_feat = self.base_forward(img_feat)
            #c4_img_feat = self.conv1x1 (c4_img_feat)
            c4 = torch.cat([c4, lbr],dim=1)
        outputs = []
        x = self.head(c4, lbr)
...
class _PSPHead(nn.Module):
    def __init__(self, nclass, norm_layer=nn.BatchNorm2d, norm_kwargs=None, **kwargs):
        ...
        self.psp = _PyramidPooling(2048, norm_layer=norm_layer, norm_kwargs=norm_kwargs)
        self.con2d = nn.Conv2d(4096, 512, 3, padding=1, bias=False)
        self.block = nn.Sequential(
           # nn.Conv2d(1026, 512, 1),
    ...
    def forward(self, x, img_feat = None):
        x = self.psp(x)
        x = self.con2d(x)
        #x = self.block(torch.cat([x, img_feat],dim=1))
        x = self.block(x)
        return x
